...
**Format:** `plans/{date}-your-plan-name/` (`{date}` format from `.gk.json` config (`plan.dateFormat`) injected via BeforeAgent hook.)
...
node .gemini/scripts/set-active-plan.cjs plans/{date}-plan-name
```
