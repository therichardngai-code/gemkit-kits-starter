# gk-scout-block.ps1 - PowerShell implementation for blocking heavy directories
# GEMINI-KIT ADAPTATION from Claude-kit scout-block.ps1
#
# Reads patterns from .gkignore file (defaults: node_modules, __pycache__, .git, dist, build)
#
# Blocking Rules:
...
# Determine script directory and .gemini folder for .gkignore lookup
# Script is at .gemini/hooks/gk-scout-block/gk-scout-block.ps1, so go 2 levels up to .gemini/
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$hooksDir = Split-Path -Parent $scriptDir
$geminiDir = Split-Path -Parent $hooksDir
$gkignoreFile = Join-Path $geminiDir ".gkignore"

# Default blocked patterns
$blockedPatterns = @('node_modules', '__pycache__', '\.git', 'dist', 'build')

# Read patterns from .gkignore if it exists
if (Test-Path $gkignoreFile) {
    $patterns = Get-Content $gkignoreFile |
