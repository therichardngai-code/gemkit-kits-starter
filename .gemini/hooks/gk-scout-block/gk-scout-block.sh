#!/bin/bash
# gk-scout-block.sh - Bash implementation for blocking heavy directories
#
...
# Determine script directory for .gkignore lookup
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Look for .gkignore in .gemini/ folder (2 levels up from .gemini/hooks/gk-scout-block/)
GEMINI_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
GKIGNORE_FILE="$GEMINI_DIR/.gkignore"

# Parse JSON and extract all relevant parameters using Node.js
# Output format: "ALLOWED" or "BLOCKED:pattern1,pattern2,..."
CHECK_RESULT=$(echo "$INPUT" | GKIGNORE_FILE="$GKIGNORE_FILE" node -e "
const fs = require('fs');
...
try {
  ...
  // Read patterns from .gkignore file
  const gkignorePath = process.env.GKIGNORE_FILE;
  let blockedPatterns = ['node_modules', '__pycache__', '.git', 'dist', 'build']; // defaults

  if (gkignorePath && fs.existsSync(gkignorePath)) {
    const content = fs.readFileSync(gkignorePath, 'utf-8');
