...
/**
 * gk-scout-block.js - Cross-platform hook dispatcher for Gemini-kit
 *
 * Blocks access to directories listed in .gemini/.gkignore
 ...
 *
 * Configuration:
 * - Edit .gemini/.gkignore to customize blocked patterns (one per line, # for comments)
 *
 * Platform Detection:
 * - Windows (win32): Uses PowerShell via gk-scout-block.ps1
 * - Unix (linux/darwin): Uses Bash via gk-scout-block.sh
...
try {
  ...
  if (platform === 'win32') {
    ...
    const psScript = path.join(scriptDir, 'gk-scout-block', 'gk-scout-block.ps1');
    ...
    const bashScript = path.join(scriptDir, 'gk-scout-block', 'gk-scout-block.sh');
